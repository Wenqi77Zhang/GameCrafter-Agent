...
| Accessible operation | active workspace is programmatically exposed, keyboard focus is visible, status messages are announced, destructive controls are disabled until confirmed; the current task and first recommended NTE action stay above diagnostic detail | mouse-only use is not required for core journeys; diagnostics default closed and mobile tabs remain one horizontally scrollable rail instead of wrapping | Vitest, Chromium desktop/mobile and manual keyboard check |
