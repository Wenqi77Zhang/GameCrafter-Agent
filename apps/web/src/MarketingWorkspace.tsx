...
type Signal = { id: string; source_name: string; source_url: string; observed_at: string; region: string; signal_type: string; title: string; keywords: string[]; metric_name: string | null; metric_value: number | null; notes: string | null };
...
const copy = {
  "zh-CN": {
    eyebrow: "MARKETING · M2/M3", title: "用可追溯趋势完成选题，而不是让模型猜热点。", intro: "手工记录授权公开页面上的观察值；系统不会抓取 TikTok，也不会把录入数据冒充实时 API。",
  ...
  en: {
    eyebrow: "MARKETING · M2/M3", title: "Choose a topic from traceable trends, not model guesses.", intro: "Record observations from authorized public pages. GameCrafter does not scrape TikTok or present manual data as a live API.",
...
export function MarketingWorkspace({ projectId, language }: { projectId: string; language: Language }) {
  ...
  const load = useCallback(async () => {
    const [snapshotPayload, taskPayload, signalPayload] = await Promise.all([
      api<{ items: Snapshot[] }>(`/api/projects/${projectId}/knowledge-snapshots`), api<{ items: Task[] }>(`/api/projects/${projectId}/marketing-tasks`), api<{ items: Signal[] }>(`/api/projects/${projectId}/trend-signals`),
    ]);
    setSnapshots(snapshotPayload.items); setTasks(taskPayload.items); setSignals(signalPayload.items);
  ...
  return <div className="marketing-workspace">
    ...
    <div className="marketing-setup-grid">
      ...
      <form className="panel" onSubmit={addSignal}><div className="panel-heading"><span>02</span><div><h2>{t.signal}</h2><p>{t.signalHint}</p></div></div><div className="form-grid marketing-form-grid">
    ...
    <section className="trend-ledger"><div className="list-heading"><h2>{t.signal}</h2><span>{signals.length}</span></div>{signals.length === 0 ? <div className="empty-state">{t.noSignals}</div> : <div className="signal-grid">{signals.map((item) => <article className="signal-card" key={item.id}><div className="card-meta"><span>{item.region}</span><span>{item.signal_type}</span><span>{formatDate(item.observed_at, language)}</span></div><h3>{item.title}</h3><a href={item.source_url} target="_blank" rel="noreferrer">{item.source_name}</a><p>{item.metric_name ? `${item.metric_name}: ${item.metric_value}` : "Metric not recorded"}</p></article>)}</div>}<button className="secondary-button" type="button" disabled={!taskId || !signals.length || busy !== null} onClick={() => void analyze()}>{busy === "analysis" ? t.working : t.analyze}</button></section>
